...
<center>
	...
	<div id="toolbar" >
	<form action="TraiteReservationButtons" method="post">
	<button id="add" name="button" class="btn btn-primary" type="submit" value="add">Ajouter</button>
	
	<button id="removeAll" name="button" class="btn btn-danger" type="submit" value="removeAll"> Tout supprimer</button>
	</form>
		<button id="edit" name="button" class="btn btn-secondary"  value="edit" >Modifier</button>
	
	<button id="remove" name="button" class="btn btn-warning"  value="remove">Supprimer</button>
	<div  class="select" >
	...
	>
		<thead>
			...
			<tr>
				...
				<th data-field="idreservation" data-sortable="true" data-filter-control="input" >ID reservation</th>
				...
				<th data-field="numchambre" data-sortable="true" data-filter-control="input" >Numero Chambre</th>
				<th data-field="typechambre" data-sortable="true"  data-filter-control="select" >Type Chambre</th>
				<th data-field="nbpers" data-sortable="true"  data-filter-control="input">Nb personnes</th>
				<th data-field="datedebut" data-sortable="true"  data-filter-control="select">Date debut</th>
				<th data-field="datefin" data-sortable="true"  data-filter-control="select">Date fin</th>
...
$('#edit').on('click',function(){
	var ids=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.idreservation});
	var nm=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.nom});
	var pm=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.prenom});
	var tr=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.type});
	var nc=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.numchambre});
	var tc=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.typechambre});
	var nbp=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.nbpers});
	var dd=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.datedebut});
	var df=$.map(table.bootstrapTable('getSelections'),function(row){
		 return row.datefin});
	...
	let datefin= df[0];
		 ...
		 $.ajax({
			 ...
			 success: function(data){
				 ...
				 window.location.href="ModifierReservation.jsp?id="+id+"&nom="+nom+"&prenom="+prenom+"&typeres="+type+"&numhambre="+numchambre+"&typechambre="+typechambre+"&nbpers="+nbpers+"&datedebut="+datedebut+"&datefin="+datefin;
			 }
		 }
				 );
